...
class BaseCommentNode(template.Node):
    ...
    def lookup_content_type(token, tagname):
        try:
            ...
            raise template.TemplateSyntaxError(
                "Third argument in %r must be in "
                "the format 'app.model'" % tagname
            )
    ...
    def __init__(self, ctype=None, object_pk_expr=None,
        ...
        self.comment_model = commentary.get_model()
...
class CommentFormNode(BaseCommentNode):
    ...
    def get_form(self, context):
        ...
        if obj:
            return commentary.get_form()(obj)
        else:
            return None
...
def comment_form_target():
    ...
    """
    return commentary.get_form_target()
...
def safe_comment(comment):
    """
    If ``COMMENTS_ALLOW_HTML`` is True, mark the comment as safe.
    Otherwise, strip any HTML tags.
    """
    if commentary.COMMENTS_ALLOW_HTML:
        return mark_safe(linebreaks(comment))
    else:
        return linebreaks(comment, autoescape=True)
