...
moderation options::
    ...
    from commentary.moderation import CommentModerator, moderator
...
class CommentModerator(object):
    ...
    def _get_delta(self, now, then):
        ...
        if now < then:
            raise ValueError(
                'Cannot determine moderation rules because '
                'date field is set to a value in the future'
            )
        return now - then
    ...
    def allow(self, comment, content_object, request):
        ...
        if self.auto_close_field and self.close_after is not None:
            ...
            days_delta = self._get_delta(timezone.now(), close_after_date).days
            if close_after_date is not None:
                return days_delta <= self.close_after
        return True
    ...
    def moderate(self, comment, content_object, request):
        ...
        if self.auto_moderate_field and self.moderate_after is not None:
            mod_after_date = getattr(content_object, self.auto_moderate_field)
            days_delta = self._get_delta(timezone.now(), mod_after_date).days
            if mod_after_date is not None:
                return days_delta >= self.moderate_after
        return False
...
class Moderator(object):
    ...
    def connect(self):
        ...
        signals.comment_will_be_posted.connect(
            self.pre_save_moderation, sender=get_model()
        )
        signals.comment_was_posted.connect(
            self.post_save_moderation, sender=get_model()
        )
    ...
    def register(self, model_or_iterable, moderation_class):
        ...
        for model in model_or_iterable:
            if model in self._registry:
                err = "The model '%s' is already being moderated"
                raise AlreadyModerated(err % model._meta.model_name)
    ...
    def unregister(self, model_or_iterable):
        ...
        for model in model_or_iterable:
            if model not in self._registry:
                err = "The model '%s' is not currently being moderated"
                raise NotModerated(err % model._meta.model_name)
